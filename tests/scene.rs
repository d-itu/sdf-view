...
fn render(renderer: &mut Renderer, sdf: &str, options: RenderOptions) -> Result<TestImage, Error> {
    ...
    let view = renderer.render(sdf, options)?;
    Ok(TestImage {
        width,
...
fn camera_and_lighting_affect_rendering() {
    ...
    let default_image = render(&mut renderer, sphere, options).unwrap();
    let wide = render(
        &mut renderer,
        sphere,
        RenderOptions {
    ...
    let upright = render(&mut renderer, raised_sphere, options).unwrap();
    let inverted = render(
        &mut renderer,
        raised_sphere,
        RenderOptions {
    ...
    .unwrap();
    let sample = |image: &TestImage, y: usize| image.pixels[(y * 97 + 48) * 4];
